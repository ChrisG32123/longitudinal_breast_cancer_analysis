#!/usr/bin/env python3
"""
By Chris Gerlach, May 24, 2025 (Updated May 30, 2025)

Parallelized creation of subplot visuals of tumor masks overlaid on DCE slices with bounding-boxes
and cropped views around the ROI for ISPY2 DCE-MRI series.
"""
import json
import shutil
import tempfile
...
FULL_DIR           = VIS_DIR / "full_slices"
CROPPED_DIR        = VIS_DIR / "cropped_slices"
# —————————————————————————————————————————————————————————————————————————
...
def extract_segmentation_masks(seg_img: sitk.Image) -> tuple[sitk.Image, sitk.Image]:
    ...
    tumor_mask       = (arr % 2 == 1).astype(np.uint8)  # TUMOR MASK == Odd numbers
    full_roi_mask    = ((arr == 1) | (arr == 17)).astype(np.uint8)  # REGION OF INTEREST == 1 for tumorous in bounding box, 17 for non-tumorous in bounding box
    def toSitk(m):
...
def visualize_full_slices(patient_id, time_label, frames, tumour_mask, bbox_mask):
    """Plot entire DCE slices with tumor mask and bounding-box overlayed."""
    FULL_DIR.mkdir(parents=True, exist_ok=True)
    ARRs = [sitk.GetArrayFromImage(f) for f in frames]
    TARR = sitk.GetArrayFromImage(tumour_mask)
    BARR = sitk.GetArrayFromImage(bbox_mask)
    depth = BARR.shape[0]
    slices = list(range(0, depth, SLICE_STEP_FULL))

    # compute bounding-box coordinates
    nz = np.nonzero(BARR)
    if nz[0].size == 0:
        print("Empty ROI, skipping full visualization.")
        return
    ...
    fig, axes = plt.subplots(len(slices), len(ARRs),
                             ...
                             squeeze=False)
    fig.suptitle(f"{patient_id} — {time_label} (Full)", fontsize=16)
    ...
    for r, z in enumerate(slices):
        for c, arr in enumerate(ARRs):
            ...
            # overlay tumor mask
            ax.imshow(TARR[z], cmap='Reds', alpha=0.4, interpolation='nearest', vmin=0, vmax=1)
            ax.contour(TARR[z], levels=[0.5], colors='plasma')
            # draw ROI bounding box
            rect = Rectangle((x0, y0), width, height,
                             fill=False, linewidth=1.5, edgecolor='yellow')
            ax.add_patch(rect)
            ax.axis('off')
            if r == 0:
                ax.set_title(f"Frame {c+1}", fontsize=10)
            if c == 0:
                ax.set_ylabel(f"slice {z}", rotation=0,
                              labelpad=40, va='center', fontsize=10)

    plt.tight_layout(rect=(0, 0, 1, 0.96))
    fig.savefig(FULL_DIR / f"{patient_id}_{time_label}_full.png")
    plt.close(fig)


def visualize_cropped_slices(patient_id, time_label, frames, tumour_mask, bbox_mask):
    """Plot cropped DCE slices around ROI with tumor mask overlayed."""
    ...
    nz = np.nonzero(BARR)
    if nz[0].size == 0:
        print("Empty ROI, skipping cropped visualization.")
        return
    z0, y0, x0 = np.min(nz, axis=1)
    z1, y1, x1 = np.max(nz, axis=1)

    # select z-slices within bounding box
    slices = list(range(int(z0), int(z1)+1, SLICE_STEP_CROP))

    fig, axes = plt.subplots(len(slices), len(ARRs),
                             figsize=(3*len(ARRs), 3*len(slices)),
                             squeeze=False)
    fig.suptitle(f"{patient_id} — {time_label} (Cropped)", fontsize=16)

    for r, z in enumerate(slices):
        for c, arr in enumerate(ARRs):
            ax = axes[r][c]
            crop = arr[z][y0:y1, x0:x1]
            mask_crop = TARR[z][y0:y1, x0:x1]
            ax.imshow(crop, cmap='gray', interpolation='nearest')
            # overlay tumor mask
            ax.imshow(mask_crop, cmap='Reds', alpha=0.4, interpolation='nearest', vmin=0, vmax=1)
            ax.contour(mask_crop, levels=[0.5], colors='plasma')
            ax.axis('off')
            if r == 0:
                ax.set_title(f"Frame {c+1}", fontsize=10)
            if c == 0:
    ...
    plt.tight_layout(rect=(0, 0, 1, 0.96))
    fig.savefig(CROPPED_DIR / f"{patient_id}_{time_label}_cropped.png")
    plt.close(fig)


def process_patient(patient_id, info):
    for tp in info["Time Labels"]:
        dce_frames = read_dce_series_from_patient_info(patient_id, Path(tp["DCE Zip Path"]))
        seg_img    = read_seg_series_from_patient_info(patient_id, Path(tp["Mask Zip Path"]))
        tum_mask, bbox_mask = extract_segmentation_masks(seg_img)
        dces_rrs, tum_rrs, bbx_rrs = [], [], []
        for f in dce_frames:
            b_rs, t_rs, d_rs = reorient_and_resample(bbox_mask, tum_mask, f)
            dces_rrs.append(d_rs)
            tum_rrs.append(t_rs)
            bbx_rrs.append(b_rs)
        visualize_full_slices(patient_id, tp["Label"], dces_rrs, tum_rrs[-1], bbx_rrs[-1])
        visualize_cropped_slices(patient_id, tp["Label"], dces_rrs, tum_rrs[-1], bbx_rrs[-1])
...
def main():
    ...
    with open(VALID_PATIENT_INFO, 'r') as f:
        info = json.load(f)
    # Parallelize across patients
    with concurrent.futures.ProcessPoolExecutor() as executor:
        executor.map(process_pair, info.items())
