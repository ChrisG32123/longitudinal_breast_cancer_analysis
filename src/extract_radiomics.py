...
def extract_segmentation_masks(seg_img: sitk.Image) -> list[sitk.Image]:
    ...
    # Checked with test_reorient.py. Segmentation is correct!
    tumor_mask = ((arr == 0) | (arr == 1)).astype(np.uint8)
    bounding_box_mask = ((arr == 1) | (arr == 17)).astype(np.uint8)
...
p.add_argument("--out-dir", type=Path, required=True,
               help="Output directory for the radiomics CSV files")
args = p.parse_args()
...
outfile = args.out_dir / f"radiomics_{patient_id}.csv"
